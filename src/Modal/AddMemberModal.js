...
const AddMemberModal = ({open, control,team}) => {
      ...
      const {data:peoples,isLoading,isError,error}=useGetUserQuery(query,{
      skip:!open
      })
      ...
      const handleSubmit=()=>{
        ...
        let content=null;
          ...
          }else if(!isLoading&&!isError&&peoples?.length===0){
            content=<div className='text-red-500'>Member is not found!</div>
          }else if(!isLoading&&!isError&&peoples?.length>0){
        content=<Fragment>
